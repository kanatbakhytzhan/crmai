...
- **POST /api/chatflow/webhook** — приём входящего JSON от ChatFlow: извлекаются jid и текст, формируется ответ (OpenAI или «Принял: …»), отправка через ChatFlow send-text. Всегда возвращает `{"ok": true}`; ошибки логируются.
  - **AI вкл/выкл:** если у первого активного tenant `ai_enabled=false` или чат в mute — автоответ не отправляется (входящие и лиды сохраняются).
  - **Команды mute (текст, регистр не важен):** `/stop` / `stop` — отключить автоответ в этом чате; `/start` / `start` — включить в этом чате; `/stop all` / `stop all` — отключить для всех чатов инстанса; `/start all` / `start all` — включить для всех. Ответы: «Ок. Я отключил автоответ в этом чате. Лиды будут сохраняться.» и аналогично для остальных команд.
